...
export function FilterTipeEventDropdown({ activeFilter, onFilter }) {
    const [isOpen, setIsOpen] = useState(false);
    const dropdownRef = useRef(null);
    ...
    return (
        <div className="relative w-fit" ref={dropdownRef}>
            <button
                onClick={() => setIsOpen(!isOpen)}
                className="border-neutral/30 relative flex items-center gap-2 rounded-lg border bg-white p-2.5 transition hover:bg-neutral-50 lg:rounded-xl lg:p-3"
            >
                <IconLsiFilterOutline className="text-neutral aspect-square h-5 w-5" />
                <span className="font-body text-neutral text-sm leading-none font-normal lg:text-base">
                    Tipe Acara
                ...
                {/* Indikator titik biru jika filter sedang aktif */}
                {activeFilter !== "ALL" && (
                    <span className="absolute -top-1 -right-1 flex h-2.5 w-2.5">
                        <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-blue-500"></span>
                    </span>
