...
class ParticipantCommunicationController extends Controller
{
    public function sendQr(Participant $participant)
    {
        if ($participant->metode_kehadiran !== 'OFFLINE') {
            return redirect()->route('dashboard')->with('error', 'QR hanya untuk peserta offline.');
        }

        if (! $participant->email_primary) {
            return redirect()->route('dashboard')->with('error', 'Peserta ini belum memiliki email utama yang valid.');
        }
        ...
        try {
            ...
            return redirect()->route('dashboard')->with('error', $this->buildMailErrorMessage($exception, 'QR'));
        }

        return redirect()->route('dashboard')->with('success', 'Email QR berhasil dikirim ke '.$participant->nama_lengkap.'.');
    }

    public function sendZoom(Request $request, Participant $participant)
    {
        if ($participant->metode_kehadiran !== 'ONLINE') {
            return redirect()->route('dashboard')->with('error', 'Link Zoom hanya untuk peserta online.');
        }
        ...
        if ($zoomLink === '') {
            return redirect()->route('dashboard')->with('error', 'Isi link Zoom terlebih dahulu sebelum mengirim.');
        }
        ...
        try {
            ...
            return redirect()->route('dashboard')->with('error', $this->buildMailErrorMessage($exception, 'Zoom'));
        }

        return redirect()->route('dashboard')->with('success', 'Email link Zoom berhasil dikirim ke '.$participant->nama_lengkap.'.');
    }
